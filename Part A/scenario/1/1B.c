...
void main() {
    ...
    printf("\n Modify the transmitted frame below to simulate errors, or enter as is.");
    printf("\n Enter the received message in binary: \n");
    scanf("%s", recv);
    
    if(crc(input, recv, gp)) {
        printf("\n Error in data transmission has occurred (Errors detected)\n");
    } else {
        printf("\n No error in data (Message is intact)\n");
    }
    getch();
}
